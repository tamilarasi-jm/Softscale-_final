import { Calculator, Moon, Sun, Download, Save, History, HelpCircle, ArrowLeft, Network, DollarSign, BarChart3, GitCompare, GanttChart } from "lucide-react";
...
export const AppHeader = ({ darkMode, onToggleDarkMode, onBackToHome, results, projectParams }: AppHeaderProps) => {
  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      doc.text("Generated by Vibe Coding Estimation", 14, doc.internal.pageSize.height - 10);
    ...
    <header className="border-b border-border bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/60 sticky top-0 z-10">
      <div className="flex items-center justify-between px-6 py-4">
        <div className="flex items-center gap-4">
          {onBackToHome && location.pathname !== '/' && (
            <Button 
              variant="ghost" 
              size="sm"
          ...
          <div className="flex items-center gap-2 font-bold text-lg">
            ...
            <Link to="/" className="gradient-text hover:opacity-80 transition-opacity">
              Vibe Coding Estimation
            </Link>
          </div>
        </div>
        ...
        <nav className="hidden md:flex items-center gap-1">
          <Button
            variant={location.pathname === '/estimation-tool' ? 'default' : 'ghost'}
            size="sm"
            asChild
          >
            <Link to="/estimation-tool">Estimation Model</Link>
          </Button>
          <Button
            variant={location.pathname === '/scheduling' ? 'default' : 'ghost'}
            size="sm"
            asChild
          >
            <Link to="/scheduling" className="flex items-center gap-2">
              <GanttChart className="w-4 h-4" />
              Pert Estimator
            </Link>
          </Button>
          <Button
            variant={location.pathname === '/estimator' ? 'default' : 'ghost'}
            size="sm"
            asChild
          >
            <Link to="/estimator" className="flex items-center gap-2">
              <DollarSign className="w-4 h-4" />
              Vibe Coding Estimator
            </Link>
          </Button>
          <Button
